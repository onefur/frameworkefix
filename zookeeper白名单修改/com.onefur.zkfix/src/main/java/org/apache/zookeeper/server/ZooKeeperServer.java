...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
     ...
     * notification
     */
    private class ZooKeeperServerListenerImpl implements ZooKeeperServerListener {
    ...
    public void processPacket(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        //Add the ip white list by yanglei
        //ip network segment confine
        String ip = cnxn.getRemoteSocketAddress().getAddress().getHostAddress();
        String networkSegment = ip.substring(0, ip.indexOf(".", ip.indexOf('.')+1)+1);
        //compare with white list of network segment or ip
        boolean isValid = ZookeeperWhiteListLoad.IP_SETS.contains(networkSegment) ;
        if(!isValid){
        	isValid = ZookeeperWhiteListLoad.IP_SETS.contains(ip) ;
        }
        int type = h.getType();
        if(!isValid && (OpCode.delete == type || OpCode.create == type || OpCode.setData == type)){
        	LOG.error("Illegal IP address to connect to the production of zk"+ip );
